...
export default {
  ...
  setup() {
    ...
    const getChangeColor = (forecast) => {
      ...
      if (changePercent <= 2) {
        return 'var(--blue)' // Blue for stable
      }

      if (change > 0) return 'var(--green)' // Green for increasing
      if (change < 0) return 'var(--red)' // Red for decreasing
      return 'var(--blue)' // Blue for no change
    }
